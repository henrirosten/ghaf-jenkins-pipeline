...
properties([
  parameters([
    string(name: 'IMG_URL', defaultValue: 'https://ghaf-jenkins-controller-dev.northeurope.cloudapp.azure.com/artifacts/ghaf-nightly-pipeline/build_35-commit_2ce8175bb2aec073c2c6c72f6db53ee7dce407be/packages.aarch64-linux.nvidia-jetson-orin-nx-debug/sd-image/nixos-sd-image-24.11.20240802.c488d21-aarch64-linux.img.zst', description: 'Target image url (for testing)'),
    string(name: 'DEVICE_CONFIG_NAME', defaultValue: 'orin-nx', description: 'Target device config name (for testing)'),
    string(name: 'TESTSET', defaultValue: '_boot_', description: 'Target test set (for testing)'),
    booleanParam(name: 'SKIP_IMG_DOWNLOAD', defaultValue: false, description: 'Skip image download (for testing)'),
  ])
])
    ...
    stage('Image download') {
      steps {
        script {
          ...
          if(!params.getOrDefault('SKIP_IMG_DOWNLOAD', false)) {
